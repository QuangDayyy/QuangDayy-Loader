...
if platform.system() == 'Windows':
    os.system('cls & title QuangDayy Loader')  # clear console, change title
...
keyauthapp = api(
    name = "Test app",
    ownerid = "MVihQ0ys1F",
    secret = "0ea2d7ce6db5596c750a8115c9d826dfd30c2db0c394cbc6402b051f9695853e",
    version = "1.0",
...
while True:
    ...
    import time
    client_id = "1218507111944945754"  # Enter your Application ID here.
    RPC = Presence(client_id=client_id)
    RPC.connect()
    nig = keyauthapp.user_data.username
# Make sure you are using the same name that you used when uploading the image
    start_time=time.time()
    RPC.update(large_image="512ava", details="Login as " + nig, state="In Main Menu", start=start_time)
    print(niga)
...
# Make sure you are using the same name that you used when uploading the image
        start_time=time.time()
        RPC.update(large_image="512ava", details="Login as " + nig, state="In Windows Iso section", start=start_time)
        os.system('cls')
